...
const CustomBox = ({ children, style, ...props }) => {
  return (
...
const LogRow = ({ log }: { log: any }) => {
  ...
  // TODOS los logs se pueden expandir
  const hasErrorDetails = true;
  ...
  return (
    <>
      ...
      {/* Panel expandible con detalles del log */}
      {isExpanded && (
        <CustomBox style={{
          padding: '24px',
          backgroundColor: log.status === 'SENT' ? '#f0fdf4' : '#fef2f2',
          borderLeft: `4px solid ${log.status === 'SENT' ? '#10b981' : '#ef4444'}`,
          borderBottom: '1px solid #f3f4f6'
        }}>
          <Text weight="bold" size="medium" color="black" style={{ marginBottom: '16px' }}>
            {log.status === 'SENT' ? '✅ Detalles del Envío Exitoso' : '🔍 Detalles del Error'}
          </Text>
          ...
          <CustomBox style={{ display: 'grid', gap: '16px' }}>
            ...
            {/* Información del destinatario - SIEMPRE mostrar */}
            <CustomBox>
              <Text weight="medium" size="small" color="black" style={{ marginBottom: '8px' }}>
