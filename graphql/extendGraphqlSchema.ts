  ...
  getFinancialReport: graphql.field({
    ...
    resolve: async (root, { routeIds, year }, context: Context) => {
      try {
        ...
        console.log(`[getFinancialReport] Memoria inicial: ${Math.round(initialMemory.heapUsed / 1024 / 1024)}MB / ${Math.round(initialMemory.heapTotal / 1024 / 1024)}MB`);
        
        // Reducir timeout y agregar validación de memoria
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(() => reject(new Error('Reporte financiero timeout - demasiados datos')), 15000); // 15 segundos
        });
        
        const reportPromise = (async () => {
          // Validar límite de rutas para evitar sobrecarga de memoria
          if (routeIds.length > 4) {
            throw new Error('Máximo 4 rutas permitidas para evitar problemas de memoria');
          }
          ...
          // Obtener cuentas de gasolina una sola vez (consulta pequeña)
          ...
          // Inicializar estructura de datos mensual optimizada
          const monthlyData: { [key: string]: any } = {};
          
          // Función para crear objeto de mes ultra-optimizado (solo datos esenciales)
          const createMonthData = () => ({
            // Solo datos básicos para reducir memoria
            totalExpenses: 0,
            generalExpenses: 0,
            nomina: 0,
            comissions: 0,
          ...
          // Inicializar solo los meses que se van a procesar
          for (let month = 1; month <= 12; month++) {
            const monthKey = month.toString().padStart(2, '0');
            monthlyData[monthKey] = createMonthData();
          }

          // 1. Obtener transacciones de nómina primero (sin límite para asegurar cálculos correctos)
          const payrollTransactions = await context.prisma.transaction.findMany({
            where: {
              route: { id: { in: routeIds } },
              date: { gte: yearStart, lte: yearEnd },
              type: 'EXPENSE',
              expenseSource: { in: ['NOMINA_SALARY', 'EXTERNAL_SALARY', 'VIATIC'] }
            },
            select: {
              amount: true,
              type: true,
          ...
          // 2. Obtener otras transacciones con límite para controlar memoria
          const otherTransactions = await context.prisma.transaction.findMany({
            where: {
              route: { id: { in: routeIds } },
              date: { gte: yearStart, lte: yearEnd },
              OR: [
                ...
                { 
                  type: 'EXPENSE',
                  expenseSource: { notIn: ['NOMINA_SALARY', 'EXTERNAL_SALARY', 'VIATIC'] }
                }
              ]
            },
            select: {
            ...
            take: 25000 // Límite optimizado para controlar memoria
          });

          // 3. Combinar todas las transacciones
          const transactions = [...payrollTransactions, ...otherTransactions];
          

          // 2. Obtener préstamos relevantes con consulta ultra optimizada
          const loans = await context.prisma.loan.findMany({
            where: {
              lead: {
          ...
          // 3. Procesar transacciones agrupadas por mes en chunks para controlar memoria
          const transactionChunkSize = 1000;
          for (let i = 0; i < transactions.length; i += transactionChunkSize) {
            const transactionChunk = transactions.slice(i, i + transactionChunkSize);
            ...
            if (transaction.type === 'EXPENSE') {
              ...
              if (transaction.incomeSource === 'CASH_LOAN_PAYMENT' || 
                ...
                monthData.incomes += profit; // Solo la ganancia, como en la versión original
            ...
            // Limpiar chunk después de procesarlo
            transactionChunk.length = 0;
            
            // Garbage collection cada 5 chunks de transacciones
            if (i % (transactionChunkSize * 5) === 0 && global.gc) {
              global.gc();
            }
          }

          // 4. Preprocesar datos de préstamos para optimizar cálculos mensuales
          ...
          // 5. Calcular cartera y métricas de préstamos por mes (ultra optimizado)
          let cumulativeCashBalance = 0;
          
          // Procesar meses de uno en uno para reducir memoria al máximo
          for (let month = 1; month <= 12; month++) {
            console.log(`[getFinancialReport] Procesando mes ${month}`);
            const monthKey = month.toString().padStart(2, '0');
            const monthStart = new Date(year, month - 1, 1);
            const monthEnd = new Date(year, month, 0, 23, 59, 59, 999);
            ...
            // Procesar TODOS los préstamos sin chunking para cálculos correctos
            ...
            data.operationalCashUsed = data.totalExpenses;
            data.totalInvestment = data.totalExpenses - data.loanDisbursements;
            ...
            // Limpieza adicional cada 3 meses
            if (month % 3 === 0 && global.gc) {
              global.gc();
            }
            
            // Limpiar variables locales del mes
            activeLoans = 0;
            overdueLoans = 0;
            deadLoans = 0;
            renewedLoans = 0;
            badDebtAmount = 0;
            carteraMuertaTotal = 0;
            
            
            const currentMemory = process.memoryUsage();
            console.log(`[getFinancialReport] Memoria después del mes ${month}: ${Math.round(currentMemory.heapUsed / 1024 / 1024)}MB / ${Math.round(currentMemory.heapTotal / 1024 / 1024)}MB`);
            
            // Verificar límite de 512MB
            if (currentMemory.heapUsed > 512 * 1024 * 1024) {
              console.warn(`[getFinancialReport] ADVERTENCIA: Memoria excede 512MB (${Math.round(currentMemory.heapUsed / 1024 / 1024)}MB)`);
            }
          }
        ...
        // monthlyData ya está dentro del scope del reportPromise, no es accesible aquí
        
        // Las variables están dentro del scope del reportPromise, no son accesibles aquí
        ...
        console.log(`[getFinancialReport] Memoria final: ${Math.round(finalMemory.heapUsed / 1024 / 1024)}MB / ${Math.round(finalMemory.heapTotal / 1024 / 1024)}MB`);
        
        // Verificar límite final de 512MB
        if (finalMemory.heapUsed > 512 * 1024 * 1024) {
          console.warn(`[getFinancialReport] ADVERTENCIA FINAL: Memoria excede 512MB (${Math.round(finalMemory.heapUsed / 1024 / 1024)}MB)`);
        }
