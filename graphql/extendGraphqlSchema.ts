...
async function generateCreditsWithDocumentErrorsReport(doc: any, context: Context, routeIds: string[] = []) {
  try {
    ...
    // ✅ USAR FUNCIÓN UNIFICADA DESDE EL SERVICIO
    const { generateCreditsWithDocumentErrorsReport: unifiedFunction } = require('../admin/services/reportGenerationService');
    
    // Generar PDF usando la función unificada
    const pdfBuffer = await unifiedFunction(context, routeIds);
    
    if (pdfBuffer) {
      console.log(`✅ PDF generado exitosamente por función unificada: ${pdfBuffer.length} bytes`);
      
      // Escribir el PDF al documento
      doc.save();
      doc.addContent(pdfBuffer);
      doc.restore();
      
      console.log('✅ PDF integrado correctamente al documento');
    } else {
      console.error('❌ No se pudo generar el PDF usando la función unificada');
      doc.fontSize(12).text('❌ Error: No se pudo generar el PDF del reporte', { align: 'center' });
    }
    
  } catch (error) {
    console.error('❌ Error usando función unificada para generar reporte:', error);
