...
// Query para obtener créditos elegibles para cartera muerta
export const loansForDeadDebtQuery = graphql.field({
...
// Query para obtener resumen por localidad
export const deadDebtSummaryQuery = graphql.field({
  type: graphql.list(graphql.JSON),
  args: {
    weeksSinceLoan: graphql.arg({ type: graphql.nonNull(graphql.Int) }),
    weeksWithoutPayment: graphql.arg({ type: graphql.nonNull(graphql.Int) }),
  ...
  resolve: async (_, { weeksSinceLoan, weeksWithoutPayment, routeId }, context) => {
    try {
      ...
      const loansData = await loansForDeadDebtQuery.resolve(_, { weeksSinceLoan, weeksWithoutPayment, routeId }, context);
...
// Query para obtener cartera muerta por mes
export const deadDebtByMonthQuery = graphql.field({
  type: graphql.list(graphql.JSON),
  args: {
...
// Mutación para marcar créditos como cartera muerta
export const markLoansDeadDebtMutation = graphql.field({
  type: graphql.JSON,
  args: {
...
// Mutación para quitar el estatus de cartera muerta
export const removeDeadDebtStatusMutation = graphql.field({
  type: graphql.JSON,
  args: {
    loanIds: graphql.arg({ type: graphql.nonNull(graphql.list(graphql.nonNull(graphql.ID))) })
  },
