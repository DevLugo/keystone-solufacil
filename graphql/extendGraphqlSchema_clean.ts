...
import { carteraMuerta2Extensions } from './carteraMuerta2Extensions';
...
export const extendGraphqlSchema = graphql.extend(base => {
  return {
    mutation: {
      ...
      // ✅ MUTACIONES PARA CARTERA MUERTA
      markLoansDeadDebt: graphql.field({
        type: graphql.JSON,
        args: {
          loanIds: graphql.arg({ type: graphql.nonNull(graphql.list(graphql.nonNull(graphql.ID))) }),
        ...
        // ✅ MUTACIONES PARA CARTERA MUERTA 2
        markLoansDeadDebt2: graphql.field({
          ...
          resolve: async (_, { loanIds, badDebtDate }, context) => {
            try {
              const result = await context.prisma.loan.updateMany({
                where: { id: { in: loanIds } },
                data: { badDebtDate: new Date(badDebtDate) }
              });
        ...
        removeDeadDebtStatus2: graphql.field({
          ...
          resolve: async (_, { loanIds }, context) => {
            try {
              ...
              console.error('Error al remover cartera muerta:', error);
              return {
                success: false,
                message: 'Error al remover cartera muerta',
                updatedCount: 0,
                errors: [error instanceof Error ? error.message : 'Error desconocido']
              };
            }
      ...
      deadDebtSummary: graphql.field({
        ...
        resolve: async (_, { weeksSinceLoan, weeksWithoutPayment, routeId }, context) => {
          try {
            ...
            eligibleLoans.forEach((loan: any) => {
