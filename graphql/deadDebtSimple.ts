...
// Extensiones simples para cartera muerta que se pueden agregar al schema existente
export const deadDebtSimpleExtensions = {
  // Queries
  ...
  deadDebtSummary: graphql.field({
    ...
    resolve: async (_, { weeksSinceLoan, weeksWithoutPayment, routeId }, context) => {
      try {
        const loansData = await deadDebtSimpleExtensions.loansForDeadDebt.resolve(_, { weeksSinceLoan, weeksWithoutPayment, routeId }, context);
