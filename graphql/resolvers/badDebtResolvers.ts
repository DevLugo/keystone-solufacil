  ...
  resolve: async (source, { loanIds, deadDebtDate }, context: Context) => {
    try {
      ...
      return JSON.stringify({
        success: true,
        message: `${result.count} créditos marcados como cartera muerta`,
        count: result.count
      });
    } catch (error: any) {
      console.error('Error al marcar créditos como cartera muerta:', error);
      return JSON.stringify({
        success: false,
        message: 'Error al marcar créditos como cartera muerta: ' + error.message
      });
    }
  }
});
...
export const loansForDeadDebt = graphql.field({
  ...
  resolve: async (source, { weeksSinceLoanMin, weeksSinceLoanMax, weeksWithoutPaymentMin, weeksWithoutPaymentMax, routeId, localities, badDebtStatus }, context: Context) => {
    try {
      ...
      const processedLoans: DeadDebtLoan[] = [];
      ...
      const summary: DeadDebtSummary[] = Array.from(summaryMap.entries()).map(([locality, data]) => ({
        locality,
    ...
    try {
      ...
      for (const loan of loans) {
        ...
        if (monthlyData[monthKey]) {
          const processedLoan: DeadDebtLoan = {
            id: loan.id,
            borrower: {
              fullName: loan.borrower.personalData.fullName || 'Sin nombre',
              clientCode: loan.borrower.personalData.clientCode || 'Sin código'
      ...
      for (const monthKey in monthlyData) {
        ...
        const uniqueClients = new Set(month.loans.map(loan => loan.borrower.clientCode));
        const uniqueRoutes = new Set(month.loans.map(loan => loan.lead.route));
        
        month.summary = {
          totalDeuda: month.loans.reduce((sum, loan) => sum + loan.pendingAmountStored, 0),
          totalCarteraMuerta: month.loans.reduce((sum, loan) => sum + loan.badDebtCandidate, 0),
      ...
      const result: MonthlyDeadDebtData = {
        year,
        months,
        routesInfo: routes
      };
